...
func _process(delta: float) -> void:
	...
	var target_velocity := input_vector * _travel_speed * speed_multiplier
	var base_rate := acceleration if input_vector.length() > 0.0 else deceleration
	var rate := base_rate * speed_multiplier
...
func apply_profile(wisp_color: Color, new_size_multiplier: float, new_speed_multiplier: float) -> void:
	color_tint = wisp_color
...
func _clamp_to_viewport() -> bool:
	...
	var margin := glow_radius * 0.7 * size_multiplier
	var hit_edge := false
...
func _tinted(color: Color) -> Color:
	return Color(
		color.r * color_tint.r,
		color.g * color_tint.g,
		color.b * color_tint.b,
		color.a
	)
...
func _draw() -> void:
	draw_set_transform(Vector2.ZERO, 0.0, Vector2.ONE * size_multiplier)
	...
	_draw_glow(Vector2.ZERO, glow_radius * 1.45 * pulse, _tinted(Color(1.0, 0.72, 0.39, 0.075)))
	_draw_glow(Vector2.ZERO, glow_radius * 0.82 * pulse, _tinted(Color(1.0, 0.86, 0.61, 0.14)))
	_draw_glow(Vector2.ZERO, glow_radius * 0.38 * pulse, _tinted(Color(1.0, 0.95, 0.79, 0.19)))
	...
	for particle in _trail_particles:
		...
		_draw_glow(particle_pos, particle_radius * 4.0, _tinted(Color(1.0, 0.84, 0.55, particle_alpha * 0.28)))
		draw_circle(particle_pos, particle_radius, _tinted(Color(1.0, 0.95, 0.78, particle_alpha)))
	...
	for mote in _ambient_motes:
		...
		_draw_glow(
			...
			_tinted(Color(1.0, 0.92, 0.72, float(mote["alpha"]) * flicker))
		)

	var tail_offset := -motion_direction * (2.0 + motion_energy * 4.5)
	_draw_glow(tail_offset, (core_radius + 5.0) * pulse, _tinted(Color(1.0, 0.82, 0.55, 0.07 + motion_energy * 0.035)))
	_draw_glow(shimmer_offset, core_radius * 2.5 * pulse, _tinted(Color(1.0, 1.0, 0.97, 0.11)))
	draw_circle(Vector2.ZERO, (core_radius + 0.8) * pulse, _tinted(Color(1.0, 0.95, 0.83, 0.2)))
	draw_circle(Vector2.ZERO, core_radius * pulse, _tinted(Color(1.0, 0.99, 0.94, 0.96)))
