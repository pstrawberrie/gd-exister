...
import { createParticlePrototype, destroyParticleEmitters, getBloodtideWaveY, setParticleSceneTime } from './particles/particlePrototypes.js';
...
function gameUpdate() {
    sceneTime += 1 / 60;
    setParticleSceneTime(sceneTime);
}
...
function gameRender() {
    if (activeParticleId === 'heaven')
        ...
        drawBloodtideBackground();
}

function gameRenderPost() {
    if (activeParticleId === 'bloodtide')
        drawBloodtideOcean();
}
...
function drawBloodtideBackground() {
    // Keep the atmospheric red haze behind the particle field.
    drawCircle(vec2(0, 3.4), 5.6, rgb(0.26, 0.02, 0.03, 0.018));
    drawCircle(vec2(0, -0.6), 7.8, rgb(0.34, 0.03, 0.04, 0.024));
}

function drawBloodtideOcean() {
    ...
    // Draw the ocean after LittleJS renders its particle objects. This puts the
    // falling blood rain behind the translucent water so it appears to enter
    // the sea instead of sitting on top of it.
    const columns = [];
    for (let x = leftX; x <= rightX; x += step) {
        const waveY = getBloodtideWaveY(x, surge);
        columns.push({ x, waveY });

        // Semi-transparent body still lets the submerged rain remain visible.
    ...
    // Glowing red crest remains above both the water and submerged rain.
    ...
    // Darker underbody also sits above the rain, deepening it once submerged.
